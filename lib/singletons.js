...
var SingletonFactory = exports.SingletonFactory = function SingletonFactory(Type) {
  ...
  var customArgs = options.customArgs && new Map(options.customArgs);
  ...
  var spreadArgs = function spreadArgs(array, arg) {
    ...
    if (spreadableTypes.has(type)) {
      var _customArgs$get = customArgs.get(type),
          spread = _customArgs$get.spread,
          shallowSpread = _customArgs$get.shallowSpread;

      newArray = spread === true ? Array.from(arg).reduce(spreadArgs, []) : spread !== undefined ? spread(arg).reduce(spreadArgs, []) : shallowSpread === true ? Array.from(arg) : shallowSpread(arg);
    } else {
      newArray = [arg];
    }
      ...
      convertedArgs = _args.reduce(spreadArgs, []).map(function (arg) {
        ...
        if (customArgs.has(type)) {
          var _customArgs$get2 = customArgs.get(type),
              ...
              spread = _customArgs$get2.spread,
              shallowSpread = _customArgs$get2.shallowSpread;
